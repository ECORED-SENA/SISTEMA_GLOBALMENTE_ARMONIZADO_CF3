<template lang="pug">
.curso-main-container.introduccion
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5.mb-5
  
    .titulo-principal
      .titulo-principal__numero
        .h3
          i.fas.fa-info
      .h3 Introducción

    p.mb-4 Durante el desarrollo del ciclo de vida del producto químico, incluyendo sus etapas de manufactura, transporte, comercialización e incluso manipulación y uso, el almacenamiento y transporte interno corresponden a elementos fundamentales para la gestión adecuada y segura de los productos químicos. Por ello, es importante tener en cuenta orientaciones técnicas que garanticen el mantenimiento de un ambiente de trabajo seguro y, la implementación de medidas y controles que protejan a los trabajadores, a la comunidad y en general al medio ambiente de los peligros que presentan los productos utilizados.

    .cajon.color-primario.p-4.mb-4
      p Estas orientaciones técnicas, además promueven el sostenimiento de las especificaciones de calidad y desempeño establecidas para los productos, en pro del beneficio de los clientes y de los consumidores finales. 

    p.mb-4 Este componente brinda un panorama sobre los elementos, orientaciones técnicas normativas, medidas de prevención que son importantes tener en cuenta como recomendaciones durante el almacenamiento y transporte interno de los productos químicos.

</template>

<script>
export default {
  name: 'Introduccion',
  data: () => ({
    // variables de vue
  }),
}
</script>

<style lang="sass" scoped>
.introduccion
</style>
